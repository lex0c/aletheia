...
Três categorias, e a diferença entre elas decide o que você pode concluir:

```text
AO VIVO, no host suspeito — pergunta ao kernel que talvez esteja mentindo
...
bpftool           # o canônico para eBPF: prog, link, map, objeto pinado (§35.4)

INSTALADO ANTES — vigia em runtime, como o auditd (§34.7)
LKRG              # Linux Kernel Runtime Guard (Openwall): faz hash do .text do kernel e
                  # de estruturas críticas e alerta quando alguma muda. O mais próximo de
                  # "detector de hook" pronto e mantido. Só serve se já estava lá
Tyton             # módulo de kernel focado em hook de syscall, módulo oculto e netfilter

CONCLUSIVO — offline, sobre dump de memória, sem chamar syscall
Volatility 3      # check_syscall, check_modules, hidden_modules, check_afinfo (§35.6)
```
```bash
...
sudo bpftool prog show; sudo bpftool link show; sudo ls -la /sys/fs/bpf/
```

> **O que separa as três categorias:** a primeira roda **sobre** o kernel suspeito — resultado negativo não absolve (§35.8). A segunda só existe se você instalou antes do incidente, e por isso pertence à §34.7 e à cadência da §40.1, não à resposta. A terceira é a única que não depende da boa-fé do suspeito, porque lê estrutura de kernel direto do dump em vez de chamar syscall.

> **Sobre eBPF:** `bpftool` é a ferramenta certa e não tem substituto prático. Se for automatizar, a enumeração é via syscall `bpf()` e existe biblioteca Go pura (`cilium/ebpf`) — mas o que você constrói é **conveniência**, não capacidade nova: nada ali detecta o que o `bpftool` não detecta.
...
> **E se quiser detecção em runtime, ela se instala aqui — não no incidente.** LKRG (§32) verifica continuamente a integridade do `.text` do kernel e de estruturas críticas, e alerta quando algo é modificado. É a única categoria de ferramenta que enxerga o hook **no momento em que ele acontece**, em vez de tentar achá-lo depois com as ferramentas que o próprio hook já controla. Como o auditd (§34.7): vale muito, e só se já estava ligado.
